...
Choose Create database and then configure the following options:
...
In the Instance configuration section for the DB instance class section, choose Burstable classes (includes t classes), and choose db.t3.medium from the dropdown list.
...
<i class="fas fa-sticky-note" style="color:#ff6633" aria-hidden="true"></i> **Note:** You can encrypt your Amazon RDS instances and snapshots at rest by enabling the encryption option for your RDS DB instance. Data that is encrypted at rest includes the underlying storage for a DB instance, its automated backups, read replicas, and snapshots.
...
In this task, you use the yum package manager to install the MariaDB client and then configure the Amazon EC2 Linux instance to connect to the Aurora database.
 ...
 Command: To install the MariaDB client, run the following command. The MariaDB client is what you use in later steps to connect to the Aurora instance that you just created.
...
Complete!   
```
Using a different browser tab, go back to the AWS Management Console and in the search bar, search for and choose RDS.
...
Next, log into the database.
 ...
 Copy edit: In the following command, replace <endpoint_goes_here> with the endpoint that you copied to your text editor.
...
Once the command is updated, copy the command to your clipboard.
 ...
 Command: Return to the Session Manager browser tab that was used to connect to the Command Host. To connect to the Aurora instance, run the command you had copied in the previous step.
...
In this task, you learn how to create a table in a database, load data, and run a query.
 ...
 Command: To list the available databases, run the following command.
...
MySQL [world]>  
```
 Command: To create a new table in the world database, run the following command.
...
MySQL [world]>
 Command: To insert new records into the country table that you just created, run the following commands.
...
MySQL [world]>
```
 Command: To query the table, run the following SELECT statement.
